...
namespace Arrays
{
    class Program
    {
        ...
        public static Complex MaxModulus(Complex[] array)
        {
            ...
            {
                if (result == null || item.Modulus > result.Modulus)
                {
                    result = item;
                }
            }
        ...
        {
            return array.Aggregate("[", (old, next) => old += next.ToString() + ", ", result => result.Remove(result.Length-2) + "]");
        }
